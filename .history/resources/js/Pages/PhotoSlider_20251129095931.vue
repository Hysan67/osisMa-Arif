<template>
  <div class="w-full">
    ...
    >
      ...
      <!-- Tombol kiri/kanan (desktop only) -->
      <button
        @click="prev"
        class="hidden md:flex absolute left-2 top-1/2 -translate-y-1/2 bg-white bg-opacity-60 hover:bg-opacity-100 rounded-full p-2 shadow"
      >
        ‹
      </button>
      <button
        @click="next"
        class="hidden md:flex absolute right-2 top-1/2 -translate-y-1/2 bg-white bg-opacity-60 hover:bg-opacity-100 rounded-full p-2 shadow"
      >
        ›
      </button>
    </div>
