...
<template>
  <div class="min-h-screen bg-white">
    <div class="max-w-5xl mx-auto px-4 py-16">
      <router-link to="/event" class="text-osisBlue hover:underline mb-8 inline-block font-light">&larr; Kembali ke Daftar Event</router-link>
      ...
      <article v-else>
        ...
        <div class="rounded-2xl overflow-hidden mb-16 shadow-sm">
          <div class="aspect-video bg-gray-200 flex items-center justify-center">
            <span class="text-white text-xl font-light">Dokumentasi: {{ event.title }}</span>
          </div>
        </div>

        <!-- Konten -->
        <div class="prose prose-lg max-w-none">
          <h1 class="text-5xl font-light text-osisBlue mb-4">{{ event.title }}</h1>
          <p class="text-gray-500 text-lg mb-10">{{ event.date }}</p>

          <!-- Tema -->
          <div v-if="event.theme" class="mb-12">
            <div class="pl-6 border-l-2 border-osisGold">
          ...
          <div class="mt-16 pt-8 border-t border-gray-100">
            <p class="text-gray-700 mb-4">
              ...
              <span class="font-medium text-osisBlue">OSIS MA Ma’arif Udanawu</span>
              mengucapkan:
            </p>
            <p class="text-3xl font-light text-osisBlue">
            ...
            <div class="flex items-center gap-3 mb-10">
              <button
                @click="shareEvent"
                class="flex items-center gap-2 px-4 py-2 rounded-full bg-osisBlue text-white hover:bg-osisBlue/90 transition-colors"
              >
              ...
              <button
                @click="copyLink"
                class="flex items-center gap-2 px-4 py-2 rounded-full border border-osisBlue text-osisBlue hover:bg-osisBlue hover:text-white transition-colors"
              >
