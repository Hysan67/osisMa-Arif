          ...
          <div class="mt-16 pt-8 border-t border-gray-100">
            ...
            <div class="relative mt-10" v-if="event.photos && event.photos.length">
              ...
              <div class="flex justify-center mt-4 space-x-2">
                <button
                  v-for="(p, k) in event.photos"
                  :key="k"
                  @click="goToPhoto(k)"
                  class="w-3 h-3 rounded-full"
                  :class="currentPhoto === k ? 'bg-blue-600' : 'bg-gray-400'"
                ></button>
              </div>
            </div>
...
import PhotoSlider from 'resources/js/Pages/PhotoSlider.vue'
...
function nextPhoto() {
  ...
  setTimeout(() => {
    currentPhoto.value =
      (currentPhoto.value + 1) % event.value.photos.length
