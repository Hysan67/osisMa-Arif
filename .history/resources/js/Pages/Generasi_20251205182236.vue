      ...
      <div class="space-y-24">
        <div v-for="(gen, i) in generations" :key="i" class="md:flex items-start gap-10" :class="{ 'flex-row-reverse': i % 2 === 1 }">
          <div class="md:w-2/5 mb-8 md:mb-0 flex justify-center">
            <div class="w-40 h-40 rounded-full bg-gray-200 flex items-center justify-center text-4xl text-gray-500" data-aos="flip-left" data-aos-delay="500">
              {{ gen.initials }}
            </div>
          </div>
          <div class="md:w-3/5">
            <h2 class="text-3xl font-light text-blue-600 mb-4" data-aos="fade-left" data-aos-delay="500">{{ gen.period }}</h2>
            <h3 class="text-xl font-medium text-gray-900 mb-1" data-aos="fade-right" data-aos-delay="500">{{ gen.name }}</h3>
            <p class="text-gray-600 mb-4"data-aos="fade-left" data-aos-delay="500">{{ gen.role }}</p>
            <p class="text-gray-800 mb-6 leading-relaxed" data-aos="fade-up" data-aos-delay="500">{{ gen.desc }}</p>
            <ul class="space-y-2">
            <li v-for="(item, j) in gen.highlights" :key="j" class="flex items-start">
              <span class="text-blue-600 mr-3 mt-1" data-aos="zoom-out" data-aos-delay="500">•</span>
              <span class="text-gray-800 font-light" data-aos="fade-left" data-aos-delay="500">{{ item }}</span>
            </li>
          </ul>

          <!-- Carousel Foto Generasi -->
          <div class="relative mt-10" v-if="gen.photos && gen.photos.length">
            <div class="bg-gray-200 aspect-video rounded-xl overflow-hidden flex items-center justify-center" data-aos="flip-left" data-aos-delay="500">
              <img
            ...
            <!-- Dots -->
            <div class="flex justify-center mt-4 space-x-2" data-aos="fade-up" data-aos-delay="500">
              <button
