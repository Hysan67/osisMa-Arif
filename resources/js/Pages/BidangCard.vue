...
<template>
  ...
  >
    <div class="relative">
      <img 
        :src="bidang.image || 'https://via.placeholder.com/400x300?text=No+Image'" 
        :alt="bidang.name" 
      ...
      <div class="absolute bottom-0 left-0 right-0 p-5 text-white z-10 text-left">
        ...
        <p class="text-sm opacity-90 mt-1 line-clamp-2">{{ bidang.desc }}</p>
      </div>
    </div>
  </div>
  ...
  >
    ...
    >
      ...
      <div class="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center">
        <h2 class="text-xl font-bold text-blue-600">{{ bidang.name }}</h2>
        <button
          @click="closeModal"
      ...
      <div class="p-6">
        ...
        <div class="mb-6">
          <img 
            :src="bidang.image || 'https://via.placeholder.com/600x400?text=No+Image'" 
            :alt="bidang.name" 
        ...
        <div class="mb-6">
          ...
          <p class="text-gray-700 leading-relaxed">{{ bidang.desc || 'Tidak ada deskripsi.' }}</p>
        </div>
