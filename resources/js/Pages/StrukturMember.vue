...
const fetchData = async () => {
    ...
    try {
        ...
        // Filter hanya anggota dengan status 'aktif'
        allMembers.value = (response.data.data || response.data)
          .filter(member => member.status === 'aktif')
          .map(member => ({
            ...member,
            // Pastikan bidang ada, jika tidak beri default
            bidang: member.bidang || { nama: 'Umum' }
          }));
          
        console.log('Active members data:', allMembers.value)
    } catch (err) {
  ...
  return allMembers.value.filter(member => {
    ...
    if (filters.value.search) {
      ...
      const matchesSearch = 
        ...
        (bidangNama && bidangNama.toLowerCase().includes(searchLower)) ||
